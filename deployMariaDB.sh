...
[ $(id -u) != "0" ] && { echo "${CFAILURE}Erro: Voce deve ser o root para executar este script${CEND}"; exit 1; }
...
if [[ "$MYSQL_ROOT_PASSWORD" == "" ]]; then
    echo "${CFAILURE}Erro: MYSQL_ROOT_PASSWORD nao definida!!${CEND}";
    exit 1;
fi
if [[ "$MYSQL_NORMAL_USER_PASSWORD" == "" ]]; then
    echo "${CFAILURE}Error: MYSQL_NORMAL_USER_PASSWORD nao definida!!${CEND}";
    exit 1;
fi
...
debconf-set-selections <<< "mariadb-server-10.3 mysql-server/root-pass password ${MYSQL_ROOT_PASSWORD}"
debconf-set-selections <<< "mariadb-server-10.3 mysql-server/re-root-pass password ${MYSQL_ROOT_PASSWORD}"
...
echo "MariaDB Normal User: ${MYSQL_NORMAL_USER}"
echo "MariaDB Normal User Password: ${MYSQL_NORMAL_USER_PASSWORD}"
echo "--"
echo "--"
